...
def scrape_all(config, since=None):
    ...
    def _fetch(platform, handle):
        ...
        try:
            posts = PROFILE_FETCHERS[platform](handle)
    ...
    _enrich_comments(results, errors)
    _enrich_transcripts(results, errors)

    return results, errors


def _enrich_comments(results, errors):
    ...
    def _fetch(platform, handle, url, fetcher):
        try:
            return platform, handle, url, fetcher(url), None
...
def _enrich_transcripts(results, errors):
...
def fetch_urls(urls, pillar_tokens=None):
