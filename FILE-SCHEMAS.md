...
Structure definitions for the files the yt-content-radar skill reads and produces.
...
There are no credentials or config files outside it: YouTube runs on the local
`yt-dlp` binary, so there's no API key to store.

---

## brand/profile.md

**Created by:** `/yt-content-radar` first-run setup (or by hand)
**Read by:** `/yt-content-radar`
...
## Search Terms
{Persistent keyword list used by /yt-content-radar every run. These are always searched
...
## Research Channels
{YouTube channels to scrape during /yt-content-radar — competitors, inspiration,
peers. Edit any time — changes take effect on the next /yt-content-radar run.}

| Channel | Depth | Notes |
|---------|-------|-------|
| @{handle} | {subtitles/metadata} | {optional — why this channel is here} |

Depth options:
- **subtitles** — pull captions/transcript + comments and summarize (default)
- **metadata** — engagement data only, no transcript/summary (lightweight monitoring)
...
## brand/tracked-accounts/youtube.md

**Created by:** `/yt-content-radar` first-run setup (optional — user can skip)
**Read by:** `/yt-content-radar`
**Location:** `$CONTENT_HOME/brand/tracked-accounts/youtube.md`

No YAML frontmatter. Simple list format:

```markdown
# YouTube — Tracked Channels

Add channels you want to track — competitors, inspiration, peers.

| Handle | Category | Notes |
|--------|----------|-------|
| @example | Niche leader | High-quality deep dives |
```

Channels listed here (plus those in Research Channels) are scraped each run.
...
The user's evolving content taste is **not a file**. Taste lives in Claude
Code's project **auto-memory** (`~/.claude/projects/<project>/memory/`), which
Claude reads and writes on demand — see SKILL.md Step 1a (record from last run's
feedback) and 1b (recall).
...
**Created by:** `/yt-content-radar` first-run setup (or by hand)
**Updated by:** `/yt-content-radar` (each run)
**Read by:** `/yt-content-radar` (anti-cannibalization, audience requests, performance patterns)
**Location:** `$CONTENT_HOME/brand/my-content.md`

Rolling snapshot of the user's own YouTube content performance.
...
| Channel | Tracked | Avg Engagement | Best Performer | Trend |
|---------|---------|----------------|----------------|-------|
| {channel} | {count} | {avg score} | {title} ({Nx avg}) | {↑ improving / → stable / ↓ declining} |
...
| Date | Title | Performance | Trigger | Hook |
|------|-------|-------------|---------|------|
| {YYYY-MM-DD} | {title} | {Nx avg} | {1-sentence trigger explanation} | {mechanism × form} |
...
| Topic | Date | Title | Performance | URL |
|-------|------|-------|-------------|-----|
| {specific topic label} | {YYYY-MM-DD} | {title} | {Nx avg} | {url} |
...
|---------|-------|------------|------------------|--------|
| {paraphrased request} | {N similar} | "{exact comment}" | {likes} | {post URL} |

## Recent Content (Last 30 Days)
Rolling 30-day window. Older content rolls into Topics Covered only.
...
- **Performance Summary** — replaced each update. One row per channel.
...
`feed-data.json` into `skills/yt-content-radar/assets/for-you-template.html` (at the
...
**Created by:** `/yt-content-radar` (skill-generated)
...
const FEED_DATA = {
  meta: {
    ...
    subtitle: "3 channels scraped · ...",        // header subtitle line
  ...
  posts: [                                     // [] if nothing scraped. ONE flat list —
    {                                          //   tracked-account posts AND discovered
      ...
      platform: "youtube",                     // always "youtube" (drives avatar CSS class)
      ...
      engagement: {                            // engagement bar numbers — populate
        likes: 14965,                          //   the metrics the video reports;
        views: 482000,                         //   omit/null the rest
        comments: 1300                         // number | null
      },
    ...
    items: [                                   // idea cards, up to 10
      {
        ...
        brief: {
          ...
          repurposeAs: ["Shorts cut from the demo", "Community poll"]  // string array
        }
      }
    ],
...
|-------|------|---------------|
| `platform` | string | `"youtube"` (the only platform this build supports) |
...
| engagement (comments) | string \| null | Pre-formatted: `"89 likes"`, `"4 similar · 47 likes"` |
| engagement (posts) | object \| null | `{ likes, views, comments }` — each `number \| null`. Each renders with its own icon (likes = heart, views = eye, comments = reply). Populate only the metrics the video reports — null/absent metrics are omitted, not dashed. |
...
the row (so each video shows only the metrics it reports). Empty arrays `[]`
...
**next** `/yt-content-radar` run at Step 1a, which distills durable reactions into
