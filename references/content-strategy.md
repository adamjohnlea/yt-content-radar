...
description: >
  ...
  goals, or adapting content across YouTube formats (long-form / Shorts). This skill handles the
...
- **Different length** — covered as a long-form deep dive, now a Short (or vice versa)
...
### Hook Adaptation by Format

The same hook needs different execution across YouTube's two formats:
- **Long-form:** First 5-8 seconds determine retention. The hook IS the thumbnail + title + opening line working together — they must agree and escalate.
- **Shorts:** First 1-2 seconds, no thumbnail to lean on. The opening frame + spoken/on-screen hook has to stop the scroll instantly; front-load the payoff and cut all preamble.
...
### YouTube CTA Conventions

| Format | Most Effective CTA Types |
|--------|------------------------|
| Long-form | Subscribe + like + comment. The algorithm weights subscriber engagement and watch-driven comments; ask mid-video while attention is high. |
| Shorts | Subscribe + "watch the full video". Shorts are top-of-funnel — drive viewers to the long-form or the next Short, not off-platform. |
| Comment-pinned | A pinned comment with the real CTA (link, next video, question) outperforms an in-video link, which YouTube can't make clickable in-frame. |
...
Generate adaptation angles that turn one video concept into multiple YouTube
assets — a long-form video, the Shorts cut from it, and a Community post that
seeds or extends it. The goal is maximum coverage from one idea without
re-shooting from scratch.

### Per-Angle Structure

| Component | Description |
|-----------|-------------|
| **Target format** | Long-form, Shorts, or Community post |
| **Source** | Which asset it derives from (e.g. a clip of the long-form) |
| **Hook adaptation** | Adapt the source hook to the target format's conventions |
| **What to add/change** | Format-specific adjustments (length, pacing, on-screen text, CTA) |

### Format Adaptation Rules

| Source idea | To Long-form | To Shorts | To Community post |
|-------------|-------------|-----------|-------------------|
| Deep dive / tutorial | Full 8-15 min walkthrough with chapters | 30-60s "best moment" or single-tip cut | Poll: "which part should I go deeper on?" |
| Hot take / contrarian | 6-10 min argued case with evidence | The claim as a 15-30s standalone hook | Text post stating the take, gauge reaction before filming |
| Data / result | Breakdown video with on-screen visuals | "Did you know" stat as the opening frame | Image post of the key chart with a one-line takeaway |
| List / ranking | Compilation or ranked countdown | The #1 item as a teaser Short | Poll ranking the items, then make the video from the winner |

### Angle Quality

A good repurposing angle matches the target format's conventions, adapts the
hook (not copy-pastes it), and adds format-appropriate value. "Make a Short
too" is not an angle. "30-second Short built from the 4:10 demo clip, stat as
the opening frame, 'full build in the description' CTA" is.
...
| **Repurpose as** | What else can I make from this? | From Repurposing Angles — long-form / Shorts / Community adaptations |
